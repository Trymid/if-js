...
describe('calendarMonth function returns a two-dimensional array of objects', () => {
  ...
  test('Should return two-dimensional array', () => {
    expect(getCalendarMonth(daysInMonth, daysInWeek, dayOfWeek, range)).toEqual(
      answer,
    );
    expect(getCalendarMonth(daysInMonth, daysInWeek, dayOfWeek, range)).toEqual(
      answer,
  ...
  test('Should return message about error', () => {
    const dayOfWeek = 8;
    expect(getCalendarMonth(daysInMonth, daysInWeek, dayOfWeek, range)).toBe(
      'Incorrect number of days in week',
    );
  });
});
